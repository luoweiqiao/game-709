...
//记录信息
struct sangongdouGameRecord
{
	BYTE	wins[JETTON_INDEX_COUNT];	//输赢标记
	BYTE	card;						//桌面扑克
	int		index;
	sangongdouGameRecord()
	{
		memset(this,0, sizeof(sangongdouGameRecord));
	}
};
...
{
	//区域类型
	AREA_SAN_GONG = 0,			//3公类型
	AREA_DRAGON,				//龙区域
	AREA_PHOENIX,				//凤区域
	AREA_MAX,					//最大区域
};

class CGameSanGongDouTable : public CGameTable
{
public:
	CGameSanGongDouTable(CGameRoom* pRoom,uint32 tableID,uint8 tableType);
    virtual ~CGameSanGongDouTable();
...
public:
	...
	//组件变量
protected:
	CSanGongDouLogic						m_GameLogic;							//游戏逻辑
	sangongdouGameRecord              		m_record;
	vector<sangongdouGameRecord>			m_vecRecord;	                        //游戏记录
	vector<sangongdouGameRecord>			m_vecGamePlayRecord;	                //游戏记录
