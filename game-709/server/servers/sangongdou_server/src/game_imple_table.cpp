...
CGameTable* CGameRoom::CreateTable(uint32 tableID)
{
    ...
    case emROOM_TYPE_COMMON:           // 
        {
            pTable = new CGameSanGongDouTable(this,tableID,emTABLE_TYPE_SYSTEM);
        }break;
    ...
    case emROOM_TYPE_PRIVATE:          // 
        {
            pTable = new CGameSanGongDouTable(this,tableID,emTABLE_TYPE_PLAYER);
        }break;
    default:
        {
            assert(false);
...
// 三公斗游戏桌子
CGameSanGongDouTable::CGameSanGongDouTable(CGameRoom* pRoom,uint32 tableID,uint8 tableType)
...
CGameSanGongDouTable::~CGameSanGongDouTable()
{

}

bool	CGameSanGongDouTable::CanEnterTable(CGamePlayer* pPlayer)
{
...
bool    CGameSanGongDouTable::CanLeaveTable(CGamePlayer* pPlayer)
{
	if (pPlayer == NULL)
	{
...
bool    CGameSanGongDouTable::CanSitDown(CGamePlayer* pPlayer,uint16 chairID)
{    
    return CGameTable::CanSitDown(pPlayer,chairID);
}

bool    CGameSanGongDouTable::CanStandUp(CGamePlayer* pPlayer)
{
    return true;
}   

bool    CGameSanGongDouTable::IsFullTable()
{
...
void	CGameSanGongDouTable::GetTableFaceInfo(net::table_face_info* pInfo)
{
    net::sangongdou_table_info* pwar = pInfo->mutable_sangongdou();
...
//配置桌子
bool CGameSanGongDouTable::Init()
{
	if (m_pHostRoom == NULL) {
		return false;
	}

    SetGameState(net::TABLE_STATE_SANGONGDOU_FREE);
	...
	m_iArrDispatchCardPro[Pro_Index_DaSanGong] = 100;
	m_iArrDispatchCardPro[Pro_Index_XiaoSanGong] = 100;
	m_iArrDispatchCardPro[Pro_Index_HunSanGong] = 800;
	m_iArrDispatchCardPro[Pro_Index_Point] = 9000;	
...
bool CGameSanGongDouTable::ReAnalysisParam() 
{
...
void CGameSanGongDouTable::ShutDown()
{

}

//复位桌子
void CGameSanGongDouTable::ResetTable()
{
    ResetGameData();
}

// 是否能够开始游戏
bool    CGameSanGongDouTable::IsCanStartGame()
{
	bool bFlag = true;
...
void CGameSanGongDouTable::OnTimeTick()
{
	OnTableTick();
    ...
    if(m_coolLogic.isTimeOut())
    {
        switch(tableState)
        {
        case TABLE_STATE_SANGONGDOU_FREE:           // 空闲
            {                
    
            }break;
        case TABLE_STATE_SANGONGDOU_PLACE_JETTON:   // 下注时间
            {
				SetGameState(TABLE_STATE_SANGONGDOU_GAME_END);
        ...
        case TABLE_STATE_SANGONGDOU_GAME_END:       // 结束游戏
            {
    			ClearTurnGameData();
                SetGameState(TABLE_STATE_SANGONGDOU_FREE);
	...
	if(tableState == TABLE_STATE_SANGONGDOU_PLACE_JETTON && m_coolLogic.getPassTick() > 3000)
    {
       	OnChairRobotJetton();
		OnRobotJetton();
    }    
...
void CGameSanGongDouTable::OnRobotTick()
{
	uint8 tableState = GetGameState();
	int64 passtick = m_coolLogic.getPassTick();

	if (tableState == net::TABLE_STATE_SANGONGDOU_PLACE_JETTON && m_coolLogic.getPassTick() > 500)
	{
...
// 游戏消息
int CGameSanGongDouTable::OnGameMessage(CGamePlayer* pPlayer,uint16 cmdID, const uint8* pkt_buf, uint16 buf_len)
{
	if (pPlayer == NULL)
	{
    ...
    switch(cmdID)
    {
    case net::C2S_MSG_SANGONGDOU_PLACE_JETTON:  // 用户加注
        {
            ...
            net::msg_sangongdou_place_jetton_req msg;
            PARSE_MSG_FROM_ARRAY(msg);
            return OnUserPlaceJetton(pPlayer,msg.jetton_area(),msg.jetton_score());
        }break;
	case net::C2S_MSG_SANGONGDOU_CONTINUOUS_PRESSURE_REQ://
		{
...
//用户断线或重连
bool CGameSanGongDouTable::OnActionUserNetState(CGamePlayer* pPlayer,bool bConnected,bool isJoin)
{
...
//用户坐下
bool CGameSanGongDouTable::OnActionUserSitDown(WORD wChairID,CGamePlayer* pPlayer)
{    
...
// 游戏开始
bool CGameSanGongDouTable::OnGameStart()
{
	if (m_pHostRoom == NULL)
	{
		return false;
	...
	SetGameState(TABLE_STATE_SANGONGDOU_PLACE_JETTON);
	m_coolLogic.beginCooling(s_PlaceJettonTime);
    net::msg_sangongdou_start_rep gameStart;
    gameStart.set_time_leave(m_coolLogic.getCoolTick());
    SendMsgToAll(&gameStart, net::S2C_MSG_SANGONGDOU_START);
	OnTableGameStart();
    return true;
}

uint32 CGameSanGongDouTable::GetBankerUID()
{
	return 0;
}

//游戏结束
bool CGameSanGongDouTable::OnGameEnd(uint16 chairID,uint8 reason)
{
    ...
    case GER_NORMAL:		//常规结束
        {
			...
			//结束消息
            net::msg_sangongdou_game_end msg;
...
//玩家进入或离开
void  CGameSanGongDouTable::OnPlayerJoin(bool isJoin,uint16 chairID,CGamePlayer* pPlayer)
{
	uint32 uid = 0;
	if (pPlayer!=NULL) 
	{
...
// 发送场景信息(断线重连)
void    CGameSanGongDouTable::SendGameScene(CGamePlayer* pPlayer)
{
    ...
    switch(m_gameState)
    {
    case net::TABLE_STATE_SANGONGDOU_FREE:          // 空闲状态
        {
            net::msg_sangongdou_game_info_free_rep msg;
            msg.set_time_leave(m_coolLogic.getCoolTick());           
            pPlayer->SendMsgToClient(&msg,net::S2C_MSG_SANGONGDOU_GAME_FREE_INFO);
        }break;
    case net::TABLE_STATE_SANGONGDOU_PLACE_JETTON:  // 游戏状态
    case net::TABLE_STATE_SANGONGDOU_GAME_END:      // 结束状态
        {
            net::msg_sangongdou_game_info_play_rep msg;
            for(uint8 i=0;i<JETTON_INDEX_COUNT;++i){
			...
			if (m_gameState == net::TABLE_STATE_SANGONGDOU_GAME_END)
			{
				for (uint8 i = 0; i<SHOW_CARD_COUNT; ++i)
				{
					net::msg_cards* pCards = msg.add_table_cards();
            ...
            pPlayer->SendMsgToClient(&msg,net::S2C_MSG_SANGONGDOU_GAME_PLAY_INFO);
...
int64    CGameSanGongDouTable::CalcPlayerInfo(uint32 uid,int64 winScore, int64 OnlywinScore,bool isBanker)
{
...
// 重置游戏数据
void    CGameSanGongDouTable::ResetGameData()
{
	//总下注数
	memset(m_allJettonScore,0,sizeof(m_allJettonScore));
    memset(m_playerJettonScore,0,sizeof(m_playerJettonScore));
...
void    CGameSanGongDouTable::ClearTurnGameData()
{
	//总下注数
	memset(m_allJettonScore,0,sizeof(m_allJettonScore));
	memset(m_playerJettonScore, 0, sizeof(m_playerJettonScore));
...
void CGameSanGongDouTable::IsRobotOrPlayerJetton(CGamePlayer *pPlayer, bool &isAllPlayer, bool &isAllRobot) 
{
...
// 写入出牌log
void    CGameSanGongDouTable::WriteOutCardLog(uint16 chairID,uint8 cardData[],uint8 cardCount,int32 mulip)
{
...
// 写入加注log
void    CGameSanGongDouTable::WriteAddScoreLog(uint32 uid,uint8 area,int64 score)
{
    if(score == 0)
        return;
    Json::Value logValue;
...
// 写入最大牌型
void 	CGameSanGongDouTable::WriteMaxCardType(uint32 uid,uint8 cardType)
{
    Json::Value logValue;
    logValue["uid"]  = uid;
...
void    CGameSanGongDouTable::WriteGameInfoLog()
{
	// 椅子用户
...
//加注事件
bool    CGameSanGongDouTable:: OnUserPlaceJetton(CGamePlayer* pPlayer, BYTE cbJettonArea, int64 lJettonScore)
{
    //效验参数
	if (pPlayer->IsRobot() == false)
	{
	...
	if(GetGameState() != net::TABLE_STATE_SANGONGDOU_PLACE_JETTON)
	{
        net::msg_sangongdou_place_jetton_rep msg;
        msg.set_jetton_area(cbJettonArea);
        msg.set_jetton_score(lJettonScore);
        msg.set_result(net::RESULT_CODE_FAIL);
        
		//发送消息
        pPlayer->SendMsgToClient(&msg,net::S2C_MSG_SANGONGDOU_PLACE_JETTON_REP);
		return true;
	}
	...
	net::msg_sangongdou_place_jetton_broadcast broad;
	...
	SendMsgToAll(&broad, net::S2C_MSG_SANGONGDOU_PLACE_JETTON_BROADCAST);
...
bool    CGameSanGongDouTable::OnUserContinuousPressure(CGamePlayer* pPlayer, net::msg_player_continuous_pressure_jetton_req & msg)
{	
	...
	if (msg.info_size() == 0 || GetGameState() != net::TABLE_STATE_SANGONGDOU_PLACE_JETTON || pPlayer->GetUID() == GetBankerUID())
	{
		pPlayer->SendMsgToClient(&rep, net::S2C_MSG_SANGONGDOU_CONTINUOUS_PRESSURE_REP);
		return false;
	}
	//效验参数
...
int     CGameSanGongDouTable::GetProCardType()
{
...
bool	CGameSanGongDouTable::ProbabilityDispatchPokerCard()
{
	bool bIsFlag = true;
...
void CGameSanGongDouTable::GetAreaInfo(WORD iLostIndex, WORD iWinIndex,BYTE cbType[], BYTE cbWinArea[], int64 lWinMultiple[])
{
...
bool    CGameSanGongDouTable::SetControlPlayerWin(uint32 control_uid)
{
...
bool    CGameSanGongDouTable::SetControlPlayerLost(uint32 control_uid)
{
	int iLoopCount = 1024;
...
bool CGameSanGongDouTable::SetSystemBrankerWinPlayerScore()
{
	int iLoopCount = 1024;
	for (int iLoopIndex = 0; iLoopIndex < iLoopCount; iLoopIndex++)
	{
...
bool	CGameSanGongDouTable::ProgressControlPlayer()
{
...
// 福利控制
bool CGameSanGongDouTable::DosWelfareCtrl()
{
	bool bIsFalgControl = ProgressControlPlayer();
	...
	LOG_DEBUG("roomid:%d,tableid:%d,sysBankerWinPro:%d,bIsFlagControl:%d,needChangeCard:%d,bIsSysWinScore:%d", GetRoomID(), GetTableID(), sysBankerWinPro, bIsFalgControl, needChangeCard, bIsSysWinScore);
	return true;
}

// 非福利控制
bool CGameSanGongDouTable::NotWelfareCtrl()
{
	bool bIsFalgControl = ProgressControlPlayer();
	...
	LOG_DEBUG("roomid:%d,tableid:%d,m_sysBankerWinPro:%d,bIsFlagControl:%d,needChangeCard:%d,bIsSysWinScore:%d,isAwCtrl:%d,bIsStockControl:%d",
		GetRoomID(), GetTableID(), m_sysBankerWinPro, bIsFalgControl, needChangeCard, bIsSysWinScore, isAwCtrl, bIsStockControl);
	return true;
}

//发送扑克
bool    CGameSanGongDouTable::DispatchTableCard()
{
    //重新洗牌
...
//发送游戏记录
void  CGameSanGongDouTable::SendPlayLog(CGamePlayer* pPlayer)
{
    net::msg_sangongdou_play_log_rep msg;
    for(uint16 i=0;i<m_vecRecord.size();++i)
    {
        net::sangongdou_play_log* plog = msg.add_logs();
		sangongdouGameRecord& record = m_vecRecord[i];
    ...
    if(pPlayer != NULL) 
	{
        pPlayer->SendMsgToClient(&msg, net::S2C_MSG_SANGONGDOU_PLAY_LOG);
    }
	else
	{
        SendMsgToAll(&msg,net::S2C_MSG_SANGONGDOU_PLAY_LOG);
    }
}

//最大下注
int64   CGameSanGongDouTable::GetUserMaxJetton(CGamePlayer* pPlayer, BYTE cbJettonArea)
{
	int iTimer = 3;
	//已下注额
	int64 lNowJetton = 0;
...
bool    CGameSanGongDouTable::IsSetJetton(uint32 uid)
{
	for (int i = 0; i < JETTON_INDEX_COUNT; i++)
	{
...
//计算得分
int64   CGameSanGongDouTable::CalculateScore()
{
	map<uint32, int64> mpUserLostScore;
	mpUserLostScore.clear();
	m_mpUserWinScore.clear();
...
//申请条件
int64   CGameSanGongDouTable::GetApplyBankerCondition()
{
    return GetBaseScore();
}

int64   CGameSanGongDouTable::GetApplyBankerConditionLimit()
{
    return GetBaseScore()*20;
}

void    CGameSanGongDouTable::OnRobotOper()
{
    map<uint32,CGamePlayer*>::iterator it = m_mpLookers.begin();
    for(;it != m_mpLookers.end();++it)
    {
...
int64 CGameSanGongDouTable::GetRobotJettonScore(CGamePlayer* pPlayer, uint8 area)
{
...
int64 CGameSanGongDouTable::GetRobotJettonScoreRand(CGamePlayer* pPlayer, uint8 area)
{
	int64 lUserRealJetton = 100;
...
void    CGameSanGongDouTable::OnRobotStandUp()
{
    // 保持一两个座位给玩家    
...
void    CGameSanGongDouTable::AddPlayerToBlingLog()
{
    map<uint32,CGamePlayer*>::iterator it = m_mpLookers.begin();
    for(;it != m_mpLookers.end();++it)
    {
...
bool    CGameSanGongDouTable::IsInTableRobot(uint32 uid, CGamePlayer * pPlayer)
{
	for (uint32 i = 0; i<GAME_PLAYER; ++i)
	{
...
bool CGameSanGongDouTable::OnChairRobotJetton()
{
...
void	CGameSanGongDouTable::OnChairRobotPlaceJetton()
{
...
bool CGameSanGongDouTable::OnRobotJetton()
{
...
void	CGameSanGongDouTable::OnRobotPlaceJetton()
{
...
bool CGameSanGongDouTable::ActiveWelfareCtrl()
{
...
bool    CGameSanGongDouTable::SetControlPlayerWinForAW(uint32 control_uid, int64 max_win)
{
    LOG_DEBUG("enter SetControlPlayerWinForAW function. control_uid:%d max_win:%lld", control_uid, max_win);
    int iLoopCount = 99999;
    int iLoopIndex = 0;
    for (; iLoopIndex < iLoopCount; iLoopIndex++)
    {
...
void CGameSanGongDouTable::GetGamePlayLogInfo(net::msg_game_play_log* pInfo)
{
	net::msg_sangongdou_play_log_rep* pplay = pInfo->mutable_sangongdou();
	for (uint16 i = 0; i<m_vecRecord.size(); ++i)
	{
		net::sangongdou_play_log* plog = pplay->add_logs();
		sangongdouGameRecord& record = m_vecRecord[i];
		for (uint16 j = 0; j<JETTON_INDEX_COUNT; j++) {
			plog->add_seats_win(record.wins[j]);
		}
...
void CGameSanGongDouTable::GetGameEndLogInfo(net::msg_game_play_log* pInfo)
{
	net::msg_sangongdou_play_log_rep* pplay = pInfo->mutable_sangongdou();
	net::sangongdou_play_log* plog = pplay->add_logs();
	sangongdouGameRecord& record = m_record;
	for (uint16 j = 0; j<JETTON_INDEX_COUNT; j++) {
		plog->add_seats_win(record.wins[j]);
	}
	plog->set_card(record.card);
...
void CGameSanGongDouTable::OnBrcControlSendAllPlayerInfo(CGamePlayer* pPlayer)
{
	if (!pPlayer)
	{
		return;
...
void CGameSanGongDouTable::OnBrcControlNoticeSinglePlayerInfo(CGamePlayer* pPlayer)
{
	if (!pPlayer)
	{
		return;
...
void CGameSanGongDouTable::OnBrcControlSendAllRobotTotalBetInfo()
{
...
void CGameSanGongDouTable::OnBrcControlSendAllPlayerTotalBetInfo()
{
...
bool CGameSanGongDouTable::OnBrcControlEnterControlInterface(CGamePlayer* pPlayer)
{
	if (!pPlayer)
		return false;
...
void CGameSanGongDouTable::OnBrcControlBetDeal(CGamePlayer* pPlayer)
{
	if (!pPlayer)
		return;
...
bool CGameSanGongDouTable::OnBrcAreaControl()
{
...
void CGameSanGongDouTable::OnBrcFlushSendAllPlayerInfo()
{
...
bool CGameSanGongDouTable::ChangeBanker(bool bCancelCurrentBanker)
{
	return true;
}

void CGameSanGongDouTable::SendApplyUser(CGamePlayer* pPlayer)
{
	return;
}

// 设置库存输赢  add by har
bool CGameSanGongDouTable::SetStockWinLose() 
{
...
int64 CGameSanGongDouTable::GetSinglePlayerWinScore(CGamePlayer *pPlayer, uint8 cbWinArea[JETTON_INDEX_COUNT], int64 lWinMultiple[JETTON_INDEX_COUNT], int64 &lBankerWinScore) {
	if (pPlayer == NULL)
		return 0;
...
int64 CGameSanGongDouTable::GetBankerAndPlayerWinScore() 
{
